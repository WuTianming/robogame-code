...
uint32_t csb_get_distance(void)
{
  uint32_t CSB_value = 0 ;
    ...
    // 返回毫米
	return ( CSB_value*340/100/2);
}
/*
uint32_t csb_get_distance2(void)
{
    uint32_t CSB_value2 = 0 ;
	//给发射引脚一个高电平
	HAL_GPIO_WritePin(CSB_Trig2_GPIO_Port, CSB_Trig2_Pin, GPIO_PIN_SET);
	//延时10us以上
	TIM1_Delay_us(20);
	//给发射引脚一个低电平
	HAL_GPIO_WritePin(CSB_Trig2_GPIO_Port, CSB_Trig2_Pin, GPIO_PIN_RESET);
	//等待接收引脚变成高电平
	while( HAL_GPIO_ReadPin(CSB_Echo2_GPIO_Port,CSB_Echo2_Pin) == 0);
	//设置定时器初始值为0
	__HAL_TIM_SetCounter(&htim1, 0);
	//开始计时
	__HAL_TIM_ENABLE(&htim1);
	//接收完全后不再为高电平，即当接收引脚变成低电平后，停止计时，获取计数时间
    while( HAL_GPIO_ReadPin(CSB_Echo2_GPIO_Port,CSB_Echo2_Pin) == 1);
	//获取定时器的计数值,赋值操作  a = b;
